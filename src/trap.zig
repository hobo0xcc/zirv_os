...
pub export fn traphandler() void {
    ...
    try uart.out.print("Trap occured: {}, sepc: {x}\n", .{scause, csr.readSepc()});
...
pub fn disable() u64 {
    var prev = csr.readMstatus();
    csr.writeMstatus(prev & ~csr.MSTATUS_SIE);
    var mask = prev & csr.MSTATUS_SIE;
    return mask;
}

pub fn restore(mask: u64) void {
    csr.writeMstatus(csr.readMstatus() | prev);
}